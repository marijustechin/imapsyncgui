...
- Windows x64 shipped as a first-class platform (native Windows CI, not a
  cross-compile shortcut), with a self-contained `imapsync.exe` runtime
  (ADR-016), an unsigned NSIS installer (ADR-017), and full runtime/package/
  installer verification natively on a Windows x64 runner.
- `pnpm verify` passes (221 tests); macOS behavior is not regressed (native
  macOS arm64 CI passes).
- Published pre-release `v0.1.0-e2e.3` with `imapSyncGUI-0.1.0-windows-x64-setup.exe`
  (SHA-256 `770a7c72108f8b6fec27c9d9e947e6c293bae0b0d3b58d060b370fe798103fd8`);
  the public installer was re-downloaded and its SHA-256 verified.
- A physical clean-machine Windows E2E test was **not** performed and Windows is
  **not** described as clean-machine verified (`docs/e2e-windows.md`).
- TASK-010 remains incomplete and is restored as the active pending manual task.
